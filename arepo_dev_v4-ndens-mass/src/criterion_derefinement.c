...
static int derefine_criterion_default(int i)
{
...
#ifdef REFINEMENT_CGM
  {
      ...
      if(SphP[i].HighResMassCGM > HIGHRESMASSFAC * P[i].Mass)
      {
        ...
        if (n_cgs >= All.TargetForHybridRefinement)
        {
            variableTargetGasVolume = All.TargetHybridGasVolume;

            mpi_printf("\n");
            mpi_printf("DEREFINEMENT_HYBRID Derefinement: Hybrid Gas volume! \n");
            mpi_printf("DEREFINEMENT_HYBRID Derefinement: n_cgs = %g cm^-3 \n", n_cgs);
            mpi_printf("DEREFINEMENT_HYBRID Derefinement: variableTargetGasVolume = %g\n", variableTargetGasVolume);
            mpi_printf("DEREFINEMENT_HYBRID Derefinement: P[i].Mass = %g\n", P[i].Mass);
        }
        else
        {
            variableTargetGasVolume = All.TargetGasVolume;
       
            variableTargetGasMass = (All.TargetForHybridRefinement/(All.UnitDensity_in_cgs * \
            All.HubbleParam * All.HubbleParam / PROTONMASS))*(All.TargetHybridGasVolume);

        }

        if(P[i].Mass < 0.5 * variableTargetGasMass && SphP[i].Volume < 0.5 * variableTargetGasVolume * All.cf_a3inv)
        {
            
            mpi_printf("\n");
            mpi_printf("DEREFINEMENT_HYBRID Derefinement: Hybrid Gas volume, variable Mass! \n");
            mpi_printf("DEREFINEMENT_HYBRID Derefinement: Derefining! \n");
            mpi_printf("DEREFINEMENT_HYBRID Derefinement: n_cgs = %g cm^-3 \n", n_cgs);
            mpi_printf("DEREFINEMENT_HYBRID Derefinement: variableTargetGasVolume = %g\n", variableTargetGasVolume);
            mpi_printf("DEREFINEMENT_HYBRID Derefinement: variableTargetGasMass = %g\n", variableTargetGasMass);
            mpi_printf("DEREFINEMENT_HYBRID Derefinement: P[i].Mass = %g\n", P[i].Mass);
          return 1;
        }
      }
